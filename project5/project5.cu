...
int main(int argc, char *argv[])
{
        ...
        double secTotal = 0.001 * (double)msecTotal;
        double trialsPerSec = (float)NUMTRIALS / secTotal;
        double megaTrialsPerSec = trialsPerSec / 1000000.;
        fprintf(stderr, "Blocksize = %10d, Number of Trials = %10d, MegaTrials/Second = %10.4lf\n", BLOCKSIZE, NUMTRIALS, megaTrialsPerSec);
        ...
        for (int i = 0; i < NUMTRIALS; i++)
        {
                numberOfHits += hhits[i];
        }

        // compute and print the probability:

        float probability = 100.f * (float)numberOfHits / (float)NUMTRIALS;
